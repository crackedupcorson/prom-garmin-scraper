...
class Intervals:
    def __init__(self):
        ...
        self.intervals_base = "https://intervals.icu"
    ...
    def get_activities(self):
        endpoint ="/api/v1/athlete/0/activities.csv"
        url = self.intervals_base + endpoint
        resp = utils.make_request("get", url, self.intervals_api_key)
        if resp is not None:
    ...
    def compute_bike_metrics(self, df, ftp):
        ...
        if 'watts' in df.columns and ftp:
            ...
            zone_times = df.groupby(z)['dt'].sum().to_dict()
    ...
    def compute_rough_guess_bike_metrics(self, df, ftp, metadata):
        # create similar metrics for when I'm on a turbo trainer.
        # It often won't have cadence, watts or velocity, but we can infer
        # estimates from activity metadata (intervals.icu fields) and heart rate.
        # This mirrors the cleaning and guards used in compute_bike_metrics.
